...
const GroupMessage = ({message}: propType) => {
    const friendDpRef = useRef<HTMLDivElement>(null!)
    ...
    const friendProfileImageRef = useRef<HTMLImageElement>(null!);
    ...
    useEffect(() => {
      ...
      if (userProfile.id === message.sender?.id) {
        ...
        friendDpRef.current.classList.add("hidden");
        friendDpRef.current.classList.remove("flex");
        ...
        friendDpRef.current.classList.add("flex");
        friendDpRef.current.classList.remove("hidden");
      }
    }, [])


    const showFriendImage = () => {
      showImage?.();
      useStore.getState().image.src = friendProfileImageRef.current.src;
    }
...
<div ref={textBoxRef} className="w-[100%] md:px-0 px-2 mt-6  flex ld:px-4 justify-start items-center ">
  ...
  <div ref={textContainerRef}  className="md:w-[65%]  lg:w-[55%] w-[75%] relative  flex rounded-md justify-start flex-col  items-start">
    <div ref={friendDpRef} className="w-[165px] h-[20px]  flex items-end">
      <img onClick={showFriendImage} ref={friendProfileImageRef} className="w-[25px] h-[25px] rounded-full object-contain  shadow-lg cursor-pointer" src={message.sender?.profile_image ?? penguin} alt="" />
      <small className="text-gray-400 font-muli ml-2 truncate w-[130px]">{message.sender?.username}</small>
    </div>
  {
